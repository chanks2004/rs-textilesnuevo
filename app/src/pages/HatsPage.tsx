...
export function HatsPage() {
  ...
  const hatProducts = productsConfig.products.filter(p => p.category === 'hat');
  ...
  return (
    <div className="pt-20">
      ...
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-2 text-sm text-[#6A6A6A] mb-6">
            ...
            <span>/</span>
            <span className="text-[#0A0A0A]">Hats</span>
          </div>
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className="text-4xl lg:text-6xl font-bold text-[#0A0A0A] mb-6">Premium Hats</h1>
              <p className="text-lg text-[#6A6A6A] mb-8">
                Complete your brand look with our collection of dad caps, trucker hats, and structured caps. Available in 50+ colors with custom embroidery options.
              </p>
              <div className="flex flex-wrap gap-4">
                ...
                <Link to="/services" className="bg-white text-[#0A0A0A] px-8 py-4 text-base font-medium border border-gray-200 hover:border-gray-300 transition-colors rounded-full">
                  View Embroidery Options
                </Link>
              </div>
            </div>
            <div className="bg-gray-50 rounded-2xl p-8 aspect-square flex items-center justify-center">
              <img src="/images/hats/dadcap-black.jpg" alt="Premium Hats" className="max-w-full max-h-full object-contain" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 lg:py-24 bg-gray-50">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-12">
            <h2 className="text-3xl font-bold text-[#0A0A0A] mb-4">All Hats</h2>
            <p className="text-[#6A6A6A]">Browse our complete collection of premium headwear</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {hatProducts.map((product) => (
              <ProductCard key={product.id} product={product} onClick={() => setSelectedProduct(product)} onGetQuote={goToQuoteForm} />
            ))}
          </div>
          ...
          <div className="mt-20 pt-16 border-t border-gray-200">
            <div className="text-center mb-12">
              ...
              <p className="text-[#6A6A6A]">Featured hats most popular among brands and businesses</p>
            </div>
      ...
      <section className="py-16 lg:py-24 bg-[#0A0A0A]">
        <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-gray-400 max-w-2xl mx-auto mb-8">Get a custom quote for your hat order. We offer competitive pricing and fast turnaround times.</p>
          <button
            type="button"
            onClick={() => goToQuoteForm()}
